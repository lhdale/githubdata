...
// Function to fetch join date for a GitHub username
const getJoinDate = async (username) => {
  try {
    ...
    return created_at.split("T")[0]; // Extract the date portion
  } catch (error) {
    console.error(`Error fetching join date for ${username}:`, error);
    return null;
  }
};
...
const processExcel = async (filePath) => {
  ...
  const updatedData = await Promise.all(
    jsonData.map(async (row) => {
      ...
      const joinDate = await getJoinDate(username);
  ...
  XLSX.utils.book_append_sheet(updatedWorkbook, updatedWorksheet, "Join Dates");

  XLSX.writeFile(
    updatedWorkbook,
    "C:/Users/luked/OneDrive/Documents/githubjoindates.xlsx"
  );
