...
const getOpenPullRequestsForRepo = async (repository) => {
  let page = 1;
  let pullRequests = [];
  while (true) {
    try {
...
const processRepositories = async () => {
  ...
  for (const row of data) {
    ...
    try {
      const pullRequestsCount = await getOpenPullRequestsForRepo(repository);
      rowData["Number_of_Open_Pull_Requests"] = pullRequestsCount;
      result.push(rowData);
    } catch (error) {
  ...
  require("fs").writeFileSync(
    "C:/Users/luked/OneDrive/Documents/github_number_of_open_pull_requests.xlsx",
    xls,
    "binary"
  );
};
