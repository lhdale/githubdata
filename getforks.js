...
const getAllForksForRepo = async (repository) => {
  let page = 1;
  let forks = [];
  while (true) {
    try {
      const res = await makeRequest(
        `https://api.github.com/repos/${repository}/forks`,
        {
          per_page: 100,
          page: page,
        }
...
const processRepositories = async () => {
  ...
  for (const row of data) {
    ...
    try {
      const forksCount = await getAllForksForRepo(repository);
      rowData["Number_of_Forks"] = forksCount;
      result.push(rowData);
    } catch (error) {
  ...
  require("fs").writeFileSync(
    "C:/Users/luked/OneDrive/Documents/github_number_of_forks.xlsx",
    xls,
    "binary"
  );
};
...
const workbook = XLSX.readFile(
  "C:/Users/luked/OneDrive/Documents/onlyrepos.xlsx"
);
