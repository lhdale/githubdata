    ...
    while (true) {
      const response = await axios.get(
        `https://api.github.com/repos/${owner}/${repository}/commits`,
        {
          ...
          params: {
            author: username,
            since: lastMonthDate.toISOString(), // Fetch commits since last month
            until: currentDate.toISOString(), // Fetch commits until current date
...
const processExcel = async (filePath) => {
  ...
  XLSX.writeFile(
    updatedWorkbook,
    "C:/Users/luked/OneDrive/Documents/githubfocallastmonth.xlsx"
  );
