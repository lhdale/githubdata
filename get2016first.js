...
const processUsers = async () => {
  ...
  for (const row of data) {
    const username = row[0]; // username is in the first column
    ...
    for (let month = 1; month <= 4; month++) {
      try {
        ...
        rowData[`Commits_2016_${month}`] = commitCount;
      } catch (error) {
        console.error(
          `Error processing ${username} for month ${month}:`,
          error
        );
        rowData[`Commits_2016_${month}`] = "";
      }
    }
    result.push(rowData);
  }
  ...
  require("fs").writeFileSync(
    "C:/Users/luked/OneDrive/Documents/githubcommitcount2016first.xlsx",
    xls,
    "binary"
  );
};
